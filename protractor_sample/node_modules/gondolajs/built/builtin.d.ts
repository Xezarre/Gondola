...
export interface IGondola {
     ...
     * ```typescript
     *
     *  // check if a control with id "elementId" exist .
     * await gondola.checkControlExist("#elementId");
     *
     *  // check if a control containing "Login" exist.
     * await gondola.checkControlExist("Login");
     *
     *  // check if control that has the attribute "name=submit" exist.
     * await gondola.checkControlExist({name: "submit"});
     * ```
     */
    checkControlExist(element: string | ILocator): void;
    /**
     * Check if a control does not exist.
     ...
     * ```typescript
     *
     * // check if a control with the id "elementId" does not exist .
     * await gondola.checkControlNotExist("#elementId");
     *
     * // check if a control containing "Login" does not exist.
     * await gondola.checkControlNotExist("Login");
     *
     * // check if a control that has attribute "name=submit"  does not exist.
     * await gondola.checkControlNotExist({name : "submit"});
     * ```
     */
    checkControlNotExist(element: string | ILocator): void;
    /**
     * Check if the first element with given locator has a attribute whose value matches the provided one.
    ...
    /**
     * Checks that the title of a window/page is equal to provided one.
     * The verify point is considered: Passed if the values match; otherwise the test result is: Failed.
     * @param {string} title The title attribute of the current document.
     * @example
     * ```typescript
     *
     * // Check for the existence of a window that title is "Scrum Board"
    ...
    /**
     * Clears all cookies. If cookie is specified only deletes that cookie.
    ...
    /**
     * Return a Boolean value to indicate whether the specified control is found.
     * Returns a Boolean value of ``true`` if control is found. Otherwise it returns ``false``.
     * @param {string|ILocator} element located by label|name|CSS|XPath|strict locator
     * @example
     * ```typescript
     *
     * // Return value whether the control with id "button_ControlTest" exist.
    ...
    refresh(): void;
    /**
     * Create a report using mochawesome.
    ...
    /**
     * Switches frame. In the case of null a locator it reverts to the parent.
    ...
    /**
     * Waits for element to be present on the page (the default amount is 1 second).
    ...
    /**
     * Waits for an element to be removed from the DOM tree of a page (the default ammount is  1 second).
    ...
    /**
     * Check JSON value using its JSON path.
...
export interface IGondolaMobile {
    ...
    /**
     * Runs the given function when testing on Android.<br />
     *
    ...
    /**
     * Runs the given function when testing on iOS.<br />
     *
     ...
     * * ```
     */
    getCurrentBrowser(): Client & Browser;
    /**
    ...
    /**
     * Retrieve an element's boundary.
     * @param {string | ILocator} element located by accessible id, xpath,... locator
     * @example
     * ```typescript
     *
...
/**
 * Set of actions to interact with Rest APIs endpoint supported by Gondola
 * @experimental v1
 * &#62; Can be changed or removed soon
 */
